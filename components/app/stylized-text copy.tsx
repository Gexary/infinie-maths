...
export type StyleRenderer = (props: {
  children: string;
  params?: string;
}) => ReactNode;
...
export default function StylizedText({
  text,
  textStyle = stylizedTextStyle,
}: {
  text: string;
  textStyle?: StylizedTextPatterns;
}) {
    ...
    for (const [start, { end, key }] of textStyle.patternMap.entries()) {
      if (text.startsWith(start, i)) {
        ...
        if (text.startsWith(end, j)) {
          ...
          nodes.push(
            <Fragment key={nodeCount++}>
              {textStyle.styles[key]({ children: contentArr.join(""), params })}
            </Fragment>,
          );
          buffer = [];
          i = j + end.length;
...
// https://chatgpt.com/c/69484871-8dec-8330-a99a-59c9bdc0f3b2
